...
def time_computation(xPass, yPass):
	...
	if -9.00 <= flag.xR <= -7.00 and -3.70 <= flag.yR <= -3.30:
		if not(flag.start_flag) and not(flag.end_flag):
			...
			f = open("../" + str(path.directory) + "/stats/" + str(stats_path_name) + "/" + "lap_time_" + str(path.directory) + ".txt", 'a')
	...
	if -7.60 <= flag.xR <= -7.40 and -5.00 <= flag.yR <= -3.00:
		if not(flag.end_flag) and flag.start_flag:
			...
			f.close()
			if flag.dist_flag:
				f = open("../" + str(path.directory) + "/stats/" + str(stats_path_name) + "/" + "distance_travelled_" + str(path.directory) + ".txt", 'a')
